<script>
  ...
  location.subscribe((value) => {
    Promise.all([
      fetch(
        "https://mcmap.org/api/v1/query/boe_elected_officials?sort=branch,district"
      ).then((resp) => resp.json()),
      fetch(
        `https://mcmap.org/api/v1/intersect_point/view_political_districts/${$location.lnglat[0]},${$location.lnglat[1]},4326?columns=districttype,district`
      ).then((resp) => resp.json()),
    ]).then((data) => {
      officials = data[0];
  ...
  function fetchPollingLocation(precno) {
    ...
    fetch(
      `https://mcmap.org/api/v1/query/polling_locations?` + jsonToURL(params)
    )
