<script>
  ...
  function fetchParks() {
    ...
    fetch(`https://mcmap.org/api/v1/nearest/parks_all/${$location.lnglat.join(",")},4326?${jsonToURL(params)}`)
  ...
  function fetchGreenways() {
    ...
    fetch(`https://mcmap.org/api/v1/nearest/greenways_union/${$location.lnglat.join(",")},4326?${jsonToURL(params)}`)
      .then(response => response.json())
      .then(data => {
        // make records for table
        data.forEach(el => {
